-- SAHRI SKIN — Safe rerun (run this if "relation already exists" error)
-- Paste in Supabase SQL Editor and click Run. Safe to run multiple times.
...
DO $$ BEGIN
  CREATE TYPE public.order_format AS ENUM (
    'pack_standard', 'pack_gift', 'product_refill'
  );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
INSERT INTO public.products (
  ...
  ('argan-parfumee-100ml', 'Argan Parfumée', 'Huile corps & cheveux — dans ton parfum',
   'Huile naturelle corps & cheveux. À base d''argan marocain. Parfumée dans ta fragrance au choix.',
   109, '109 MAD', 'Héros — corps & cheveux, parfumé', 'best-seller',
   'products/argan-parfumee-100ml/argan-parfumee-100ml-studio.png', 'products/argan-parfumee-100ml/', 1, true, true, false),
  ('beurre-corporelle-200g', 'Beurre Corporel', 'Le beurre qui scelle ton parfum sur la peau',
   'Beurre corps naturel. Texture crémeuse veloutée. Hydratation longue durée. Parfumé dans ta fragrance.',
   119, '119 MAD', 'Héros texture — hydratation profonde', 'best-seller',
   'products/beurre-corporelle-200g/beurre-corporelle-200g-studio.png', 'products/beurre-corporelle-200g/', 2, true, true, false),
  ('gommage-corps-300g', 'Gommage Corps', 'Le gommage qui prépare ta peau à absorber le rituel',
   'Gommage corps naturel. Exfoliation douce. Prépare la peau à absorber le rituel.',
   99, '99 MAD', 'Préparation — exfoliation avant le rituel', 'rituel',
   'products/gommage-corps-300g/gommage-corps-300g-studio.png', 'products/gommage-corps-300g/', 3, false, true, false),
  ('gommage-levres-30g', 'Gommage à Lèvres', 'Lèvres douces, soignées, parfumées',
   'Gommage lèvres naturel. Sucre fin, texture fondante. Le soin visage du rituel.',
   79, '79 MAD', 'Soin visage — touche féminine, UGC', 'ugc-favorite',
   'products/gommage-levres-30g/gommage-levres-30g-studio.png', 'products/gommage-levres-30g/', 4, false, true, false),
  ('petit-savon-cadeau', 'Petit Savon', 'Le petit cadeau qui rend tout le pack spécial',
   'Savon naturel offert dans chaque pack. Parfumé dans la fragrance choisie.',
   NULL, 'Offert', 'Cadeau offert — valeur perçue', 'offert',
   'products/petit-savon-cadeau/petit-savon-cadeau-studio.png', 'products/petit-savon-cadeau/', 5, false, false, true)
ON CONFLICT (slug) DO NOTHING;
...
INSERT INTO public.packs (
  ...
  'pack-rituel-v2', 'Pack Rituel SAHRI SKIN', 'Ton rituel corps complet — dans ton parfum',
...
WHERE p.slug = 'pack-rituel-v2'
  AND pr.slug IN (
    'argan-parfumee-100ml', 'beurre-corporelle-200g', 'gommage-corps-300g',
    'gommage-levres-30g', 'petit-savon-cadeau'
  )
ON CONFLICT (pack_id, product_id) DO NOTHING;

-- ── Views ────────────────────────────────────────────────────
CREATE OR REPLACE VIEW public.catalog_products AS
SELECT id, slug, name_fr, subtitle_fr, description_short,
  price_mad, price_label, role_in_pack, badge, image_path, page_path, sort_order,
...
SELECT p.id, p.slug, p.name_fr, p.subtitle_fr, p.description_short,
  p.price_standard_mad, p.price_gift_mad, p.separate_total_mad, p.savings_mad, p.page_path,
  COALESCE(json_agg(json_build_object(
    'slug', pr.slug, 'name_fr', pr.name_fr, 'role_in_pack', pr.role_in_pack,
    'image_path', pr.image_path, 'page_path', pr.page_path,
    'is_gift', pi.is_gift, 'sort_order', pi.sort_order
  ) ORDER BY pi.sort_order) FILTER (WHERE pr.id IS NOT NULL), '[]'::json) AS items
FROM public.packs p
...
CREATE OR REPLACE FUNCTION public.submit_order(
  p_customer_name text, p_phone text, p_city text,
  p_scent_name text DEFAULT NULL, p_product_slug text DEFAULT NULL,
  p_pack_slug text DEFAULT 'pack-rituel-v2', p_format text DEFAULT 'pack_standard',
  p_notes text DEFAULT NULL
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_scent_id uuid; v_product_id uuid; v_pack_id uuid;
  v_total numeric(10,2); v_product_name text; v_order_id uuid;
  v_format public.order_format;
BEGIN
  IF length(trim(p_customer_name)) < 2 THEN RAISE EXCEPTION 'Nom invalide'; END IF;
  IF length(trim(p_phone)) < 8 THEN RAISE EXCEPTION 'Téléphone invalide'; END IF;
  IF length(trim(p_city)) < 2 THEN RAISE EXCEPTION 'Ville invalide'; END IF;
  IF p_scent_name IS NOT NULL AND length(trim(p_scent_name)) > 0 THEN
    SELECT id INTO v_scent_id FROM public.scents WHERE name_fr = trim(p_scent_name) AND is_active = true;
  END IF;
  ...
  IF p_product_slug IS NOT NULL AND length(trim(p_product_slug)) > 0 THEN
    ...
    FROM public.products WHERE slug = trim(p_product_slug) AND is_active = true AND sold_separately = true;
  ...
  ELSE
    SELECT id INTO v_pack_id FROM public.packs
    WHERE slug = COALESCE(NULLIF(trim(p_pack_slug), ''), 'pack-rituel-v2') AND is_active = true;
    IF v_pack_id IS NULL THEN RAISE EXCEPTION 'Pack introuvable'; END IF;
    SELECT CASE v_format WHEN 'pack_gift' THEN price_gift_mad ELSE price_standard_mad END INTO v_total
    FROM public.packs WHERE id = v_pack_id;
    v_product_name := (SELECT name_fr FROM public.packs WHERE id = v_pack_id);
    IF v_scent_id IS NULL THEN RAISE EXCEPTION 'Parfum requis pour le pack'; END IF;
  END IF;
  INSERT INTO public.orders (customer_name, phone, city, scent_id, scent_name, product_id, pack_id, product_name, format, total_mad, notes)
  VALUES (trim(p_customer_name), trim(p_phone), trim(p_city), v_scent_id, NULLIF(trim(p_scent_name), ''),
    v_product_id, v_pack_id, v_product_name, v_format, v_total, NULLIF(trim(p_notes), ''))
...
DROP POLICY IF EXISTS "Public read active scents" ON public.scents;
CREATE POLICY "Public read active scents" ON public.scents FOR SELECT TO anon, authenticated USING (is_active = true);
...
CREATE POLICY "Public read active packs" ON public.packs FOR SELECT TO anon, authenticated USING (is_active = true);

DROP POLICY IF EXISTS "Public read pack items" ON public.pack_items;
CREATE POLICY "Public read pack items" ON public.pack_items FOR SELECT TO anon, authenticated USING (true);
...
-- ── Verify ───────────────────────────────────────────────────
SELECT 'scents' AS table_name, count(*)::text AS rows FROM public.scents
UNION ALL SELECT 'products', count(*)::text FROM public.products
UNION ALL SELECT 'packs', count(*)::text FROM public.packs
UNION ALL SELECT 'pack_items', count(*)::text FROM public.pack_items
